...
$(document).ready(function() {
    ...
    try {
        ...
        $.each(data, function(i, datafile) {
            ...
            $.each(line_totals, function(k) {
                line_totals[k] += sum.lines[k];
            });
        });

        coverage = (line_totals.covered / line_totals.executable * 100).toPrecision(3);

        $.each(line_totals, function(k, v) {
            $('#' + k + '_total').text(v);
        });
